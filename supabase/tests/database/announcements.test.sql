...
select throws_ok($$select public.announcement_followup(current_setting('test.revision')::uuid,'all',true)$$,'42501','Announcement follow-up denied','Student cannot export follow-up');
select set_config('request.jwt.claims','{"sub":"b1000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select throws_ok($$select public.revise_announcement(current_setting('test.announcement')::uuid,'branches',array['b3000000-0000-4000-8000-000000000002'::uuid],'tr','Updated','{}')$$,'42501','Announcement access denied','Revision cannot target another Operator');
select set_config('test.new_revision',public.revise_announcement(current_setting('test.announcement')::uuid,'operator','{}'::uuid[],'tr','Updated','{}')::text,true);
select is((select count(*) from public.announcement_revisions where announcement_id=current_setting('test.announcement')::uuid),2::bigint,'Revision appends to history');
select is((select source_content from public.announcement_revisions where id=current_setting('test.revision')::uuid),'Kaynak','Old content preserved');
select is((select target_scope from public.announcement_revisions where id=current_setting('test.revision')::uuid),'branches','Old target scope preserved');
select is((select count(*) from public.announcement_acknowledgements where revision_id=current_setting('test.revision')::uuid),1::bigint,'Old acknowledgment preserved');
select is((select count(*) from public.announcement_acknowledgements where revision_id=current_setting('test.new_revision')::uuid),0::bigint,'New revision does not inherit acknowledgments');
select is(jsonb_array_length(public.announcement_followup(current_setting('test.new_revision')::uuid,'unacknowledged',true)->'recipients'),1,'Filtered export returns unacknowledged current recipients');
select set_config('request.jwt.claims','{"sub":"b1000000-0000-4000-8000-000000000002","role":"authenticated","session_id":"b5000000-0000-4000-8000-000000000001"}',true);
select throws_ok($$select public.acknowledge_announcement(current_setting('test.revision')::uuid)$$,'42501','Acknowledgment denied','Old revision cannot be acknowledged after replacement');
select lives_ok($$select public.acknowledge_announcement(current_setting('test.new_revision')::uuid)$$,'Student explicitly acknowledges new revision');
reset role;
select throws_ok($$update public.announcement_revisions set source_content='tampered' where id=current_setting('test.revision')::uuid$$,'55000','Announcement history is immutable','Even privileged content rewrites are guarded');
