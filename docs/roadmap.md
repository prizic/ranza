...
so Finance has a screen and Analytics has somewhere to read revenue from.

**Nothing creates a Reservation.** Every row the workspace shows was written by
`scripts/db-seed-dev.mjs`, so the product can check somebody in and bill them
but cannot be told they are coming. That is the shape of the next few slices,
and their order is decided by what each one unblocks:

1. **A Guest, and a Reservation somebody can create.** A Reservation carries a
   `guest_name` string and a Folio bills it. Creation is also what makes
   overlapping confirmed Reservations reachable, so the protection lands with
   the command rather than after it.
2. **Beds, floors and buildings.** `accommodation_units` is flat and its
   `capacity` column is unenforceable — see the Phase 2 row below.
3. **A Unit status lifecycle**, which is also the first outbox handler:
   check-out marks the Unit dirty.
4. **A rate.** Nothing has a price, which is why there is no night audit and why
...
| Reservations, availability, Stays, check-in, check-out, transfers | **partial** | All but transfers, and a mistaken check-in can be withdrawn from the arrivals screen ([ADR 0022](adr/0022-a-mistaken-check-in-is-reversed-not-deleted.md)) until money is posted against it — a row action, a reason, and the Reservation is arrivable again with the empty Folio closed behind it. `pnpm test:browser` presses the button. `packages/ranza/reservations`, migrations `…000900`–`…001100`, `…001300`, `…001500`, `…001800`, `…001900`, [ADR 0012](adr/0012-a-write-is-bounded-by-a-policy-not-a-check.md). Availability is an exclusion constraint, not a query. **Transfers — room moves — are refused by a column-level grant**, so building them is a deliberate act rather than a side effect. **Nothing creates a Reservation** — every one is seeded — and two confirmed Reservations may overlap on one Unit, which is unreachable until creation becomes a command and is the reason the protection belongs in that slice |
...
| Guest and Resident profiles                                       | —           | **The next structural piece.** A Reservation carries a `guest_name` string and a Folio bills it. There is no Guest record, so nothing can hold a document, a guardian, an emergency contact or a history across two Stays                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Reservation timeline, command center, Guest 360                   | **partial** | Arrivals is the first live screen — it polls through TanStack Query, scoped to the tenant ([ADR 0019](adr/0019-a-live-screen-uses-tanstack-query-scoped-to-the-tenant.md)). The timeline is not built; blueprint 18.6 specifies it in detail: filters, conflict preview, a side drawer, an action cluster                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
