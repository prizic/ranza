...
1. **Beds, floors and buildings.** `accommodation_units` is flat and its
   `capacity` column is unenforceable — see the Phase 2 row below. It is now the
   thing most in the way: a booking screen that can only offer whole rooms
   cannot sell a bed in a shared one, which is what
   [ADR 0004](adr/0004-student-residence-is-a-property-configuration.md) exists
   for.
...
| Audit, observability, notifications, data lifecycle              | **partial** | Audit done: `packages/platform/audit`, append-only by trigger, [ADR 0008](adr/0008-a-module-owns-a-schema-not-a-migration-history.md), and readable from the Workspace at `/audit-log` ([ADR 0028](adr/0028-an-organization-wide-read-is-gated-through-the-property-it-is-opened-from.md)). Observability is a stub. Notifications and data lifecycle are not built |
...
| Reservation timeline, command center, Guest 360                   | **partial** | The booking list — `front-office/reservations`, with the form that takes one — and arrivals, the first live screen — it polls through TanStack Query, scoped to the tenant ([ADR 0019](adr/0019-a-live-screen-uses-tanstack-query-scoped-to-the-tenant.md)). The timeline is not built; blueprint 18.6 specifies it in detail: filters, conflict preview, a side drawer, an action cluster                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
