...
can be built. `src/lib/screens.ts` says which are built; the rest are routes
with a stated purpose and nothing behind them.
...
so every destination appears — which they would not on a real Organization
...
| ----------------------- | --------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `today`                 | 18.4      | **built**            | `features/today/`, designed in [`docs/features/today-dashboard`](../features/today-dashboard/edge-cases.csv) and drawn in [`docs/design/mockups/today.html`](../design/mockups/today.html). The day at a Property, shaped by the permissions the viewer holds: what needs attention, four figures for their focus, today's movements or the rooms to clean first, and readiness by floor — every figure opening the list that explains it, and no balance sent to anybody without `finance.manage_folio`. Acting from it (slice 2) and the manager's activity, charges today and Organization strip (slice 3) are designed and not built.                                                                                                                                                                                                                                                        |
...
| `room-calendar`         | 18.6      | **built**, read-only | Nothing. Units against days with bookings and Stays as bars, polled like arrivals, and the named gap shown as an overlap ([`features/room-calendar/`](../features/room-calendar/edge-cases.csv)). Its drawer links to where each command lives rather than repeating it; moving a Stay waits on transfers, which a column grant refuses on purpose. Property defaults for the view are slice 2, now unblocked by #57.                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| `close-day`             | 6.4       | **built**            | `features/front-office/`. The business day waiting to be closed, what holds it up with the commands that resolve it, and the recent closes; the worker closes a quiet day by itself ([ADR 0034](../adr/0034-a-business-day-closes-after-its-cutoff.md)). Room nights need a rate (RANZ-31) and are the feature's third slice; reopening a closed day is designed and deferred.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| `configuration`         | 5.1       | **built**            | `features/configuration/`. A Property's name, currency, timezone and business-day cutoff, and the Organization's name, under `configuration.manage` ([ADR 0036](../adr/0036-a-property-is-configured-and-its-currency-is-fixed-by-its-first-folio.md)). The currency locks at the first Folio; a timezone or cutoff change previews the business date it makes. Housekeeping's inspection setting is shown here by its own component; Rooms and People are linked, not repeated. Not yet: the Organization's default language, switching capabilities, department or outlet values (`CF-DEF-*`).                                                                                                                                                                                                                                                                                                 |
| `audit-log`             | 7.4       | **built**            | `features/audit-log/`. Opened by whoever holds `audit.read` — shipped on Owner and Manager — with no commercial gate, because audit is a baseline right. Records carry the Property they happened at, and the read policy narrows to the Properties the reader reaches; a record about the whole Organization is for Organization-wide readers. Actors, Guests, rooms and roles are named at read time, times are each Property's own clock, and filters, search and paging run on the server over every record ([ADR 0031](../adr/0031-an-audit-record-carries-its-location-and-is-read-by-permission.md), after [ADR 0028](../adr/0028-an-organization-wide-read-is-gated-through-the-property-it-is-opened-from.md)). A record opens by id, so a link to one keeps working. The seed writes its demo rows raw, so the log is empty until something is checked in, booked, posted or reversed. |
