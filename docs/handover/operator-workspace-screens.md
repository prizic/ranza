...
| `audit-log`             | 7.4       | **built**            | `features/audit-log/`. The Organization's records newest first, and one record's whole reason and context, read through the Property it is opened from ([ADR 0028](../adr/0028-an-organization-wide-read-is-gated-through-the-property-it-is-opened-from.md)) — the first screen to read a platform module. The actor is a stable id: naming a colleague needs a co-member lookup, which is a migration. Records are Organization-wide because `audit.records` has no `property_id` — also a migration. The seed writes its demo rows raw, so the log is empty until something is checked in, booked, posted or reversed. |
