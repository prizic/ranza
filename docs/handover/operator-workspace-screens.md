...
can be built. Two are built; the rest are routes with a stated purpose and
nothing behind them.
...
example — read them before starting a new one.
...
| `arrivals` `departures` | 5.3       | **built**       | Nothing. The worked example.                                                                                                                                                                                                                    |
...
| `housekeeping`          | 5.4       | planned         | A Unit status lifecycle — available, occupied, dirty, inspected, blocked, out of order (18.2). `accommodation_units.status` currently allows three of those. Check-out is the natural trigger for "dirty" and deliberately does not set it yet. |
...
| `finance`               | 5.9, 5.10 | planned         | The Folio: the financial record collecting charges against a Stay. This is the next structural piece after Reservations and the one most other screens wait on. Accounting is a separate module behind an adapter.                              |
| `people`                | 5.11      | planned         | Staff records beyond membership. `organization_memberships` and `property_assignments` exist and nothing reads them yet — a read-only listing is the cheapest real screen here.                                                                 |
| `analytics`             | 5.14      | planned         | Occupancy and revenue reporting. Needs Folios for the revenue half; the occupancy half could be built on Stays today.                                                                                                                           |
