    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          // Physical, deliberately, and `shadcn migrate rtl` gets this wrong —
          // it rewrote these to start-0/end-0, which double-flips in Arabic and
          // puts a side="right" rail on the left. `side` is a physical edge in
          // shadcn's API, so what positions it has to be physical too. The
          // logical-property rule governs content; it does not govern an API
          // that names left and right.
          side === "left"
...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      {...props}
    >
      {/* Ranza: the untranslated <span className="sr-only">Toggle Sidebar</span>
          shadcn ships here was removed. This product has no fallback locale —
          an English string on a Turkish page is a defect, not a default — so
          the caller's aria-label is the accessible name. */}
      <PanelLeftIcon className="rtl:rotate-180" />
    </Button>
  );
}

/** `label` is required for the same reason the trigger's is: nothing here is English by default. */
